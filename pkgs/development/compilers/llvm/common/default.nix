    ...
    in
    {
      ...
      clang =
        if stdenv.targetPlatform.libc == null then
          tools.clangNoLibc
        else if stdenv.targetPlatform.cc == "clang" && stdenv.targetPlatform.cxxlib == "libcxx" && stdenv.targetPlatform.bintools == "llvm" && stdenv.targetPlatform.rtlib == "compiler-rt" then
          tools.clangUseLLVM
    ...
    (
      {
        ...
        compiler-rt-no-libc = callPackage ./compiler-rt {
          patches = compiler-rtPatches;
          doFakeLibgcc = stdenv.hostPlatform.rtlib == "compiler-rt";
          stdenv =
            # Darwin needs to use a bootstrap stdenv to avoid an infinite recursion when cross-compiling.
